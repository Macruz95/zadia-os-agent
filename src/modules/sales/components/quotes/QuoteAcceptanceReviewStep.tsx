/**
 * ZADIA OS - Quote to Project Review Step
 * 
 * First step of acceptance wizard: Review quote before conversion
...
import { FileText, Calendar, DollarSign, User, Building2, ChevronRight } from 'lucide-react';
...
type QuoteAcceptanceInput = z.infer<typeof quoteAcceptanceSchema>;

interface QuoteAcceptanceReviewStepProps {
  quote: Quote;
  onNext: () => void;
  onAcceptanceData: (data: QuoteAcceptanceInput) => void;
}

export function QuoteAcceptanceReviewStep({ 
  quote, 
  onNext, 
  onAcceptanceData 
}: QuoteAcceptanceReviewStepProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: {
      quoteId: quote.id,
      acceptedBy: '', // Should be filled with current user
    },
  });
